        ...
        <form method="POST" action="{{ route('roles.update', $role->id) }}" class="space-y-6">
            ...
            <x-card>
                ...
                <div class="max-w-md">
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" value="{{ old('name', $role->name) }}" required placeholder="Ex: Chef de Projet" />
            ...
            <x-card>
                <x-slot name="header">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Cochez les permissions à attribuer à ce rôle. Les permissions marquées <span class="text-red-600 font-semibold">Super Admin</span> sont réservées et ne peuvent être attribuées qu'au rôle Super Admin.</p>
                </x-slot>
                                    ...
                                    @foreach($perms as $perm)
                                        ...
                                        <div x-data="{ open: false }" class="flex items-center gap-1">
                                            <input type="checkbox" name="permission[]" value="{{ $perm->name }}" id="perm_{{ $loop->parent->index }}_{{ $loop->index }}"
                                                ...
                                                {{ $isRolePermission && !$isSuperAdminRole ? 'disabled' : '' }}
                                                {{ in_array($perm->name, old('permission', $rolePermissions ?? [])) ? 'checked' : '' }}>
                                            <label for="perm_{{ $loop->parent->index }}_{{ $loop->index }}" class="text-sm {{ $isRolePermission && !$isSuperAdminRole ? 'text-gray-400 line-through' : 'text-gray-700' }} cursor-pointer select-none">
                                                {{ $action }}
                                            </label>
                                            @if($isRolePermission && !$isSuperAdminRole)
