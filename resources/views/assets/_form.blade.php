<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ (isset($asset) && $asset->exists) ? 'Modifier Matériel : ' . ($asset ?? null)?->marque : 'Ajouter un Matériel' }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <form action="{{ (isset($asset) && $asset->exists) ? route('assets.update', $asset->id) : route('assets.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div x-data="{ isNewType: {{ old('type_materiel_id') === 'autre' ? 'true' : 'false' }} }">
                            ...
                            <select name="type_materiel_id" @change="isNewType = ($event.target.value === 'autre')" :required="!isNewType" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500">
                                ...
                                @foreach(\App\Models\TypeMateriel::all() as $type)
                                    <option value="{{ $type->id }}" @selected(old('type_materiel_id', ($asset ?? null)?->type_materiel_id ?? '') == $type->id)>{{ $type->libelle_type }}</option>
                                @endforeach
                        ...
                        <div>
                            ...
                            <x-text-input id="num_serie" name="num_serie" type="text" class="mt-1 block w-full" :value="old('num_serie', ($asset ?? null)?->num_serie ?? '')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('num_serie')" />
                        </div>
                        <div>
                            <x-input-label for="marque" value="Marque *" />
                            <x-text-input id="marque" name="marque" type="text" class="mt-1 block w-full" :value="old('marque', ($asset ?? null)?->marque ?? '')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('marque')" />
                        </div>
                        <div>
                            <x-input-label for="modele" value="Modèle *" />
                            <x-text-input id="modele" name="modele" type="text" class="mt-1 block w-full" :value="old('modele', ($asset ?? null)?->modele ?? '')" required />
                            <x-input-error class="mt-2" :messages="$errors->get('modele')" />
                        </div>
                        <div>
                            <x-input-label for="date_achat_actif" value="Date d'Achat" />
                            <x-text-input id="date_achat_actif" name="date_achat_actif" type="date" class="mt-1 block w-full" :value="old('date_achat_actif', ($asset ?? null)?->date_achat_actif?->format('Y-m-d') ?? '')" />
                            <x-input-error class="mt-2" :messages="$errors->get('date_achat_actif')" />
                        </div>
                        <div>
                            <x-input-label for="prix_achat" value="Valeur d'Achat (DHS)" />
                            <x-text-input id="prix_achat" name="prix_achat" type="number" step="0.01" class="mt-1 block w-full" :value="old('prix_achat', ($asset ?? null)?->prix_achat ?? '')" />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select name="statut_materiel" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500">
                                <option value="disponible" @selected(old('statut_materiel', ($asset ?? null)?->statut_materiel ?? '') === 'disponible')>Disponible</option>
                                <option value="attribue" @selected(old('statut_materiel', ($asset ?? null)?->statut_materiel ?? '') === 'attribue')>Attribué</option>
                                <option value="en_panne" @selected(old('statut_materiel', ($asset ?? null)?->statut_materiel ?? '') === 'en_panne')>En Panne</option>
                                <option value="reforme" @selected(old('statut_materiel', ($asset ?? null)?->statut_materiel ?? '') === 'reforme')>Réformé</option>
                            </select>
