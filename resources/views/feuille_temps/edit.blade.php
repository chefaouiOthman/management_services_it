<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <a href="{{ route('feuille_temps.index') }}" class="text-indigo-600 hover:text-indigo-900 flex items-center">
            ...
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('feuille_temps.update', $feuille->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="employe_id" name="employe_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required {{ Auth::user()->hasRole('Admin') ? '' : 'disabled' }}>
                                    @foreach($employes as $employe)
                                        <option value="{{ $employe->user_id }}" {{ old('employe_id', $feuille->employe_id) == $employe->user_id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <x-text-input id="duree_heures" name="duree_heures" type="number" step="0.5" min="0.5" max="24" class="mt-1 block w-full" value="{{ old('duree_heures', $feuille->duree_heures) }}" placeholder="Ex: 5.5" required />
                            ...
                            <div class="col-span-full">
                                ...
                                <textarea id="commentaire" name="commentaire" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">{{ old('commentaire', $feuille->commentaire) }}</textarea>
                        ...
                        <div class="flex items-center justify-end mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <x-primary-button class="ml-4">
                                {{ __('Mettre à jour la feuille de temps') }}
                            </x-primary-button>
                        </div>
                    </form>
                </div>
