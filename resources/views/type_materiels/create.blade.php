<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <x-card>
                <form action="{{ route('type_materiels.store') }}" method="POST" class="space-y-6">
                    @csrf

                    <div>
                        <x-input-label for="libelle_type" value="Titre du type *" />
                        <x-text-input id="libelle_type" name="libelle_type" type="text" class="mt-1 block w-full" :value="old('libelle_type')" required autofocus />
                        <x-input-error class="mt-2" :messages="$errors->get('libelle_type')" />
                    </div>

                    <div>
                        <x-input-label for="description_type" value="Description" />
                        <textarea id="description_type" name="description_type" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description_type') }}</textarea>
