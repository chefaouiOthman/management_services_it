        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    DÃ©tails de la Licence : {{ $licence->nom_logiciel }}
                </h2>
                <p class="text-sm text-gray-500 mt-1">ClÃ©: {{ $licence->cle_licence }}</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <x-card class="bg-{{ $isExpired ? 'red' : 'green' }}-50 border-{{ $isExpired ? 'red' : 'green' }}-100">
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ $isExpired ? 'ExpirÃ©e' : 'Valide' }}</p>
                </x-card>
                <x-card>
            ...
            <x-card>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <p class="text-sm text-gray-500">ClÃ© de Licence</p>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900 {{ $isExpired ? 'text-red-600' : 'text-green-600' }}">
                            {{ $isExpired ? 'ExpirÃ© depuis ' . $expDate->diffForHumans() : $expDate->diffInDays() . ' jours' }}
                        </p>
                    </div>
                </div>
            </x-card>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3">Date RÃ©vocation</th>
                                <th class="px-6 py-3">DurÃ©e</th>
                            </tr>
                        </thead>
                        <tbody>
    ...
    <script>
        function licenceManager() {
            return {
                ...
                revoquer(pivotId) {
                    if(!confirm('ÃŠtes-vous sÃ»r de vouloir rÃ©voquer cette licence pour cet utilisateur ?')) return;
                    ...
                    .then(data => {
                        if(data.success) {
                            ...
                            alert(data.message || 'Erreur lors de la rÃ©vocation.');
                        }
                    }).catch(err => {
