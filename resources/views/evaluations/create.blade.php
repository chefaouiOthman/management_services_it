        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('evaluations.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="session_formation_id" name="session_formation_id" required class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($sessions as $s)
                                    <option value="{{ $s->id }}" {{ old('session_formation_id') == $s->id ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="employe_id" name="employe_id" required class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($formateurs as $f)
                                    <option value="{{ $f->user_id }}" {{ old('employe_id') == $f->user_id ? 'selected' : '' }}>
                ...
                <x-card>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="note_pedagogie" name="note_pedagogie" type="number" min="0" max="5" step="1" class="mt-1 block w-full" value="{{ old('note_pedagogie') }}" required />
                            <x-input-error :messages="$errors->get('note_pedagogie')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="note_technique" value="Note Technique *" />
                            <x-text-input id="note_technique" name="note_technique" type="number" min="0" max="5" step="1" class="mt-1 block w-full" value="{{ old('note_technique') }}" required />
                ...
                <x-card>
                    ...
                    <div>
                        <textarea id="avis_textuel" name="avis_textuel" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">{{ old('avis_textuel') }}</textarea>
