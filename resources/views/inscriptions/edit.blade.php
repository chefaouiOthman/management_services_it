        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('inscriptions.update', $inscription->id) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="col-span-full">
                                ...
                                <select id="session_formation_id" name="session_formation_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($sessions as $session)
                                        <option value="{{ $session->id }}" {{ (old('session_formation_id', $inscription->session_formation_id) == $session->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-span-full">
                                <x-input-label for="statut_inscription" value="Statut" />
                                <select id="statut_inscription" name="statut_inscription" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required>
                                    <option value="valide" {{ old('statut_inscription', $inscription->statut_inscription) == 'valide' ? 'selected' : '' }}>Validé</option>
                                    <option value="annule" {{ old('statut_inscription', $inscription->statut_inscription) == 'annule' ? 'selected' : '' }}>Annulé</option>
                                    <option value="present" {{ old('statut_inscription', $inscription->statut_inscription) == 'present' ? 'selected' : '' }}>Présent</option>
                                    <option value="certifie" {{ old('statut_inscription', $inscription->statut_inscription) == 'certifie' ? 'selected' : '' }}>Certifié</option>
                                </select>
